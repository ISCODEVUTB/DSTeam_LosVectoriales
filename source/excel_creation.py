...
class ExcelPackages:
        ...
        if not os.path.exists(cls.FILE_PATH):
            ...
            ws.append(
                [
                    ...
                    package.price,
                    package.type,
                    ", ".join(package.content) if isinstance(package.content, list) else str(package.content),
                    ", ".join(package.category) if isinstance(package.category, list) else str(package.category),
                    ", ".join(package.dimension) if isinstance(package.dimension, list) else str(package.dimension),
    ...
    def show_dis(cls):
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            if row[8] == "pendiente":
                print(
                    f"{row[0]} | {row[1]} | {row[2]} | {row[3]} | {row[4]} | {row[5]} | {row[6]} | {row[7]} | Disponible"
                )
