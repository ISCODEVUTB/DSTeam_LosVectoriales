...
op_n, USERS_FILE, cod_admin = "Opción inválida", "users.txt", {123, 321, 456, 654}

def hash_password(password):
    return hashlib.sha256(password.encode()).hexdigest()

def input_user_data():
    return input("Ingrese el nombre de usuario: "), input("Ingrese la contraseña: ")

def register_user():
    user, pin = input_user_data()
    user_type = input("Ingrese el tipo de usuario (cliente, administrador, domiciliario): ").lower()
    if user_type not in {"cliente", "administrador", "domiciliario"}:
        return print("Tipo de usuario no válido.")
    
    if user_type == "administrador" and int(input("Ingrese código de administrador: ")) not in cod_admin:
    ...
    with open(USERS_FILE, "r") as file:
        for line in file:
            ...
            if user == u and p_hash == hash_password(pin):
                print(f"Autenticación exitosa. Bienvenido, {user} ({user_type}).")
                {"cliente": menu_customer, "administrador": menu_administrator, "domiciliario": menu_domiciliary}[user_type](user)
                return
...
def update_d(cls, user):
    ...
    packages_user = [(i+2, row) for i, row in enumerate(ws.iter_rows(min_row=2, values_only=True)) if row and row[0] == user]
    ...
    try:
        row_idx, package = packages_user[int(input("\nSeleccione el número del paquete que desea cambiar: ")) - 1]
    except (ValueError, IndexError):
        return print("Selección inválida.")

    opciones = ["Nombre", "Precio", "Peso", "Tipo", "Contenido", "Categoría", "Dimensiones"]
    ...
    ws.cell(row=row_idx, column=cambio + 2, value=input("Ingrese el nuevo valor: "))
